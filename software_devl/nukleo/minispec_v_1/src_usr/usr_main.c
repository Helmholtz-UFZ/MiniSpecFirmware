...
int usr_main( void )
{
	...
	HAL_UART_Receive_DMA( &hrxtx, uart3_rx_buffer.base, uart3_rx_buffer.size );

	if( data_format == DATA_FORMAT_ASCII )
	{
		tx_printf( &hrxtx, &uart3_tx_buffer, "\nstart\n" );
	}
	while( 1 )
	{
		// IR in uart module
		...
		if( uart3_CR_recvd == 0 && stream_mode == 0 )
		{
		...
		parse_extcmd( uart3_rx_buffer.base, uart3_rx_buffer.size );
		...
		case USR_CMD_READ_ITIME:
			...
			{
				tx_printf( &hrxtx, &uart3_tx_buffer, "integration time = %ld us\n", sens1.itime );
			}
			break;
...
static void send_data( uint8_t format )
{
	...
	else if( format == DATA_FORMAT_ASCII )
	{
		...
		while( rptr < sens1.data->wptr )
		{
			...
			{
				tx_printf( &hrxtx, &uart3_tx_buffer, "%05d ", *rptr );
			}
...
static void error_handler( uint8_t err )
{
	...
	case SENS_ERR_TIMEOUT:
		if( data_format == DATA_FORMAT_ASCII )
		{
			tx_printf( &hrxtx, &uart3_tx_buffer, "ERR: TIMEOUT. Please check the following:\n"
				"1. is sensor plugged ?\n"
	...
	case SENS_ERR_NO_EOS:
		if( data_format == DATA_FORMAT_ASCII )
		{
			tx_printf( &hrxtx, &uart3_tx_buffer, "ERR: NO EOS. Something went wrong, please debug manually.\n" );
		}
	...
	case SENS_ERR_EOS_EARLY:
		if( data_format == DATA_FORMAT_ASCII )
		{
			tx_printf( &hrxtx, &uart3_tx_buffer, "ERR: EOS EARLY. Something went wrong, please debug manually.\n" );
		}
